...
pub mod merkle {
    ...
    mod test {
        ...
        use crate::blockchain::{
            merkle::root,
            test_utils::{generate_keypair, signed_create_account_tx},
            transaction::Transaction,
        };

        /// Tests that the merkle root of an empty transaction list fails
        #[test]
        fn test_merkle_root_rejects_empty_list() {
            assert!(root::<Transaction>(&[]).is_err());
        }

        /// Tests that the merkle root is deterministic for the same input
        #[test]
        fn test_merkle_root_is_deterministic() -> Result<(), Box<dyn Error + Send + Sync>> {
            ...
            assert_eq!(root(&[t.clone()])?, root(&[t])?);
            Ok(())
        }

        /// Tests that reordering transactions produces a different merkle root
        #[test]
        fn test_merkle_root_is_order_sensitive() -> Result<(), Box<dyn Error + Send + Sync>> {
            let k1 = generate_keypair();
            let k2 = generate_keypair();
            let t1 = signed_create_account_tx(&k1, 0)?;
            let t2 = signed_create_account_tx(&k2, 0)?;
            assert_ne!(root(&[t1.clone(), t2.clone()])?, root(&[t2, t1])?);
            Ok(())
        }

        /// Tests that adding a transaction changes the merkle root
        #[test]
        fn test_merkle_root_changes_with_different_transactions()
        -> Result<(), Box<dyn Error + Send + Sync>> {
            let k1 = generate_keypair();
            let k2 = generate_keypair();
            let t1 = signed_create_account_tx(&k1, 0)?;
            let t2 = signed_create_account_tx(&k2, 0)?;
            assert_ne!(root(&[t1])?, root(&[t2])?);
            Ok(())
        }
    }
}
...
pub mod transaction {
    use crate::{
        blockchain::{
            ...
            ed25519::{
                public_key_to_string, signature_to_string, string_to_public_key,
                string_to_signature,
            },
            hash::{self, Hashable},
            test_utils::{generate_keypair, signed_create_account_tx},
